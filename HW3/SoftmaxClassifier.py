...
#    def __init__(self, lr = 0.03, alpha = 1, n_epochs = 20000, eps = 1.0e-5,threshold = 1.0e-10 , regularization = False, early_stopping = #True):
    ...
    def score(self, X, y=None):
        ...
        J = -(1/probabilities.shape[0])*(log_prob*y).sum() 
    ...
    def _cost_function(self,probabilities, y ):
        ...
        if self.regularization==True:
            J = -(1/probabilities.shape[0])*mul.sum() + self.alpha*(self.theta_[1:,]**2).sum()
        else:
            J = -(1/probabilities.shape[0])*mul.sum()
        return J
    ...
    def _one_hot(self,y):
        ...
        n_values = np.max(y) + 1
        # ne garder que les classes existantes dans y
        return np.eye(n_values)[y][:,list(set(y))]
    ...
    def _get_gradient(self,X,y, probas):
        ...
        theta[0,:] = 0
        if self.regularization==True:
            gradient = (1/probas.shape[0])*np.dot(X.transpose(),(probas-yhot)) + (self.alpha*2)*theta
        else:
            gradient = (1/probas.shape[0])*np.dot(X.transpose(),(probas-yhot))
        return gradient
